 ...
 * ViraWM.h
 * 
 * ViraWM, a library for the ESP8266/Arduino platform
...
// #define vwm_MDNS            // includes MDNS, also set MDNS with sethostname
// #define vwm_FIXERASECONFIG  // use erase flash fix
// #define vwm_ERASE_NVS       // esp32 erase(true) will erase NVS 
// #define vwm_RTC             // esp32 info page will include reset reasons

// #define vwm_JSTEST                      // build flag for enabling js xhr tests
...
#define vwm_NOASYNC         // esp8266 no async scan wifi
#define vwm_NOCOUNTRY       // esp8266 no country
#define vwm_NOAUTH          // no httpauth
#define vwm_NOSOFTAPSSID    // no softapssid() @todo shim
#endif
...
#warning "vwm_NOTEMP"
#define vwm_NOTEMP // disabled temp sensor, have to determine which chip we are on
#endif
...
#define vwm_WEBSERVERSHIM      // use webserver shim lib

#define vwm_G(string_literal)  (String(FPSTR(string_literal)).c_str())
    ...
    #define vwm_WIFIOPEN   ENC_TYPE_NONE
    ...
    #define vwm_WIFIOPEN   WIFI_AUTH_OPEN

    #ifndef WEBSERVER_H
        #ifdef vwm_WEBSERVERSHIM
            #include <WebServer.h>
        #else
            #include <ESP8266WebServer.h>
        #endif
...
// Pass in strings env override via vwm_STRINGS_FILE
#ifndef vwm_STRINGS_FILE
#define vwm_STRINGS_FILE "vwm_strings_en.h" // this includes constants as dependency
#endif
#include vwm_STRINGS_FILE

// prep string concat vars
#define vwm_STRING2(x) #x
#define vwm_STRING(x) vwm_STRING2(x)    

// #include <esp_idf_version.h>
#ifdef ESP_IDF_VERSION
    // #pragma message "ESP_IDF_VERSION_MAJOR = " vwm_STRING(ESP_IDF_VERSION_MAJOR)
    // #pragma message "ESP_IDF_VERSION_MINOR = " vwm_STRING(ESP_IDF_VERSION_MINOR)
    // #pragma message "ESP_IDF_VERSION_PATCH = " vwm_STRING(ESP_IDF_VERSION_PATCH)
    #define VER_IDF_STR vwm_STRING(ESP_IDF_VERSION_MAJOR)  "."  vwm_STRING(ESP_IDF_VERSION_MINOR)  "."  vwm_STRING(ESP_IDF_VERSION_PATCH)
#else 
    ...
    // esp_get_idf_version
        ...
        // #pragma message "ESP_ARDUINO_VERSION_MAJOR = " vwm_STRING(ESP_ARDUINO_VERSION_MAJOR)
        // #pragma message "ESP_ARDUINO_VERSION_MINOR = " vwm_STRING(ESP_ARDUINO_VERSION_MINOR)
        // #pragma message "ESP_ARDUINO_VERSION_PATCH = " vwm_STRING(ESP_ARDUINO_VERSION_PATCH)
        #ifdef ESP_ARDUINO_VERSION_MAJOR
        #define VER_ARDUINO_STR vwm_STRING(ESP_ARDUINO_VERSION_MAJOR)  "."  vwm_STRING(ESP_ARDUINO_VERSION_MINOR)  "."  vwm_STRING(ESP_ARDUINO_VERSION_PATCH)
        #else
        #define VER_ARDUINO_STR "Unknown"
        #endif
    #else
        #include <core_version.h>
        // #pragma message "ESP_ARDUINO_VERSION_GIT  = " vwm_STRING(ARDUINO_ESP32_GIT_VER)//  0x46d5afb1
        // #pragma message "ESP_ARDUINO_VERSION_DESC = " vwm_STRING(ARDUINO_ESP32_GIT_DESC) //  1.0.6
        // #pragma message "ESP_ARDUINO_VERSION_REL  = " vwm_STRING(ARDUINO_ESP32_RELEASE) //"1_0_6"
        #ifdef ESP_ARDUINO_VERSION_MAJOR
        #define VER_ARDUINO_STR vwm_STRING(ESP_ARDUINO_VERSION_MAJOR)  "."  vwm_STRING(ESP_ARDUINO_VERSION_MINOR)  "."  vwm_STRING(ESP_ARDUINO_VERSION_PATCH)
        #else
        #define VER_ARDUINO_STR "Unknown"
...
// #pragma message "VER_IDF_STR = " vwm_STRING(VER_IDF_STR)
// #pragma message "VER_ARDUINO_STR = " vwm_STRING(VER_ARDUINO_STR)
...
class ViraWMParameter {
  public:
    /** 
        Create custom parameters that can be added to the ViraWM setup web page
        @id is used for HTTP queries and must not contain spaces nor other special characters
    */
    ViraWMParameter();
    ViraWMParameter(const char *custom);
    ViraWMParameter(const char *id, const char *label);
    ViraWMParameter(const char *id, const char *label, const char *defaultValue, int length);
    ViraWMParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom);
    ViraWMParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement);
    ~ViraWMParameter();
    // ViraWMParameter& operator=(const ViraWMParameter& rhs);
  ...
  private:
    ViraWMParameter& operator=(const ViraWMParameter&);
    const char *_id;
    const char *_label;
    char       *_value;
    int         _length;
  ...
  protected:
    ...
    // debugging
    typedef enum {
        vwm_DEBUG_SILENT    = 0, // debug OFF but still compiled for runtime
        vwm_DEBUG_ERROR     = 1, // error only
        vwm_DEBUG_NOTIFY    = 2, // default stable,INFO
        vwm_DEBUG_VERBOSE   = 3, // move verbose info
        vwm_DEBUG_DEV       = 4, // development useful debugging info
        vwm_DEBUG_MAX       = 5  // MAX extra dev auditing, var dumps etc (MAX+1 will print timing,mem and frag info)
    } vwm_debuglevel_t;

class ViraWM
{
  public:
    ViraWM(Print& consolePort);
    ViraWM();
    ~ViraWM();
    void ViraWMInit();
    bool clearWiFiProfile();
    bool clearWiFiProfile(bool reboot = false);
    ...
    bool          addParameter(ViraWMParameter *p);

    //returns the list of Parameters
    ViraWMParameter** getParameters();
    ...
    void          setAPCallback( std::function<void(ViraWM*)> func );
    ...
    void          setDebugOutput(boolean debug, vwm_debuglevel_t level ); // log line prefix, default "*wm:"
    ...
    // set the webapp title, default ViraWM
    ...
    #if defined(ESP32) && defined(vwm_WEBSERVERSHIM)
        using vwm_WebServer = WebServer;
    #else
        using vwm_WebServer = ESP8266WebServer;
    #endif
    
    std::unique_ptr<vwm_WebServer> server;
  ...
  private:
    ...
    #ifdef ESP32
    wifi_event_id_t vwm_event_id           = 0;
    ...
    String        _title                  = FPSTR(S_brand); // app title -  default ViraWM
...
private:
    ...
    // check for arduino or system event system, handle esp32 arduino v2 and IDF
        ...
        #define vwm_ARDUINOVERCHECK ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(2, 0, 0)
        #define vwm_ARDUINOVERCHECK_204 ESP_ARDUINO_VERSION <= ESP_ARDUINO_VERSION_VAL(2, 0, 5)

        #ifdef vwm_ARDUINOVERCHECK
            #define vwm_ARDUINOEVENTS
        #else
            #define vwm_NOSOFTAPSSID
            #define vwm_NOCOUNTRY
        #endif

        #ifdef vwm_ARDUINOVERCHECK_204
            #define vwm_DISCONWORKAROUND
        #endif
    ...
    int         _paramsCount          = 0;
    int         _max_params;
    ViraWMParameter** _params    = NULL;

    boolean _debug  = true;
    String _debugPrefix = FPSTR(S_debugPrefix);

    vwm_debuglevel_t debugLvlShow = vwm_DEBUG_VERBOSE; // at which level start showing [n] level tags

    // build debuglevel support
    // @todo use DEBUG_ESP_x?
    
    // Set default debug level
    #ifndef vwm_DEBUG_LEVEL
    #define vwm_DEBUG_LEVEL vwm_DEBUG_NOTIFY
    #endif

    // override debug level OFF
    #ifdef vwm_NODEBUG
    #undef vwm_DEBUG_LEVEL
    #endif

    #ifdef vwm_DEBUG_LEVEL
    uint8_t _debugLevel = (uint8_t)vwm_DEBUG_LEVEL;
    #else 
    uint8_t _debugLevel = 0; // default debug level
    #endif

    // @todo use DEBUG_ESP_PORT ?
    #ifdef vwm_DEBUG_PORT
    Print& _debugPort = vwm_DEBUG_PORT;
    #else
    ...
    template <typename Generic>
    void        DEBUG_WM(vwm_debuglevel_t level,Generic text);
    template <typename Generic, typename Genericb>
    void        DEBUG_WM(Generic text,Genericb textb);
    template <typename Generic, typename Genericb>
    void        DEBUG_WM(vwm_debuglevel_t level, Generic text,Genericb textb);

    // callbacks
    // @todo use cb list (vector) maybe event ids, allow no return value
    std::function<void(ViraWM*)> _apcallback;
