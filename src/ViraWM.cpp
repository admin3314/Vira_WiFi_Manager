...
#ifdef ESP32
uint8_t ViraWM::_lastconxresulttmp = WL_IDLE_STATUS;
#endif
#include <EEPROM.h>
...
void saveServerIP(const char* svip) {
    int startAddress = 1; 
    int len = strlen(svip);
    for (int i = 0; i < len; i++) {
        EEPROM.write(startAddress + i, svip[i]);
    }
    EEPROM.write(startAddress + len, '\0');
    EEPROM.commit();
}

void saveGateway(const char* gateway) {
    int startAddress = 17; 
    int len = strlen(gateway);
    for (int i = 0; i < len; i++) {
        EEPROM.write(startAddress + i, gateway[i]);
    }
    EEPROM.write(startAddress + len, '\0');
    EEPROM.commit();
}

void saveSubnet(const char* subnet) {
    int startAddress = 33;
    int len = strlen(subnet);
    for (int i = 0; i < len; i++) {
        EEPROM.write(startAddress + i, subnet[i]);
    }
    EEPROM.write(startAddress + len, '\0');
    EEPROM.commit();
}
...
String readServerIP() {
    ...
    char ch;
    int i = 0;
    while ((ch = EEPROM.read(i)) != '\0' && i < 512) {
        svip += ch;
        i++;
    }
    return svip;
...
String readGateway() {
    ...
    char ch;
    int i = 0;
    while ((ch = EEPROM.read(i + 512)) != '\0' && i < 512) {
        gateway += ch;
        i++;
    }
    return gateway;
...
String readSubnet() {
    ...
    char ch;
    int i = 0;
    while ((ch = EEPROM.read(i + 1024)) != '\0' && i < 512) {
        subnet += ch;
        i++;
    }
    return subnet;
...
ViraWMParameter::ViraWMParameter() {
  ViraWMParameter("");
}

ViraWMParameter::ViraWMParameter(const char *custom) {
  _id             = NULL;
  _label          = NULL;
  _length         = 0;
  _value          = nullptr;
...
ViraWMParameter::ViraWMParameter(const char *id, const char *label) {
  init(id, label, "", 0, "", WFM_LABEL_DEFAULT);
}

ViraWMParameter::ViraWMParameter(const char *id, const char *label, const char *defaultValue, int length) {
  init(id, label, defaultValue, length, "", WFM_LABEL_DEFAULT);
}

ViraWMParameter::ViraWMParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom) {
  init(id, label, defaultValue, length, custom, WFM_LABEL_DEFAULT);
}

ViraWMParameter::ViraWMParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  init(id, label, defaultValue, length, custom, labelPlacement);
}

void ViraWMParameter::init(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  _id             = id;
  _label          = label;
...
ViraWMParameter::~ViraWMParameter() {
  ...
  _length=0; // setting length 0, ideally the entire parameter should be removed, or added to ViraWM scope so it follows
}



// ViraWMParameter& ViraWMParameter::operator=(const ViraWMParameter& rhs){
...
void ViraWMParameter::setValue(const char *defaultValue, int length) {
  if(!_id){
...
const char* ViraWMParameter::getValue() const {
  // Serial.println(printf("Address of _value is %p\n", (void *)_value)); 
  return _value;
}
const char* ViraWMParameter::getID() const {
  return _id;
}
const char* ViraWMParameter::getPlaceholder() const {
  return _label;
}
const char* ViraWMParameter::getLabel() const {
  return _label;
}
int ViraWMParameter::getValueLength() const {
  return _length;
}
int ViraWMParameter::getLabelPlacement() const {
  return _labelPlacement;
}
const char* ViraWMParameter::getCustomHTML() const {
  return _customHTML;
}


bool ViraWM::clearWiFiProfile(bool reboot)
{
 ...
 * @access public
 * @param {[type]} ViraWMParameter *p [description]
 */
bool ViraWM::addParameter(ViraWMParameter *p) {

  // check param id is valid, unless null
  if(p->getID()){
    for (size_t i = 0; i < strlen(p->getID()); i++){
       if(!(isAlphaNumeric(p->getID()[i])) && !(p->getID()[i]=='_')){
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] parameter IDs can only contain alpha numeric chars"));
        #endif
        return false;
       }
    }
  ...
  if(_params == NULL){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("allocating params bytes:"),_max_params * sizeof(ViraWMParameter*));        
    #endif
    _params = (ViraWMParameter**)malloc(_max_params * sizeof(ViraWMParameter*));
  }
  ...
  if(_paramsCount == _max_params){
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("Updated _max_params:"),_max_params);
    DEBUG_WM(vwm_DEBUG_DEV,F("re-allocating params bytes:"),_max_params * sizeof(ViraWMParameter*));    
    #endif
    ViraWMParameter** new_params = (ViraWMParameter**)realloc(_params, _max_params * sizeof(ViraWMParameter*));
    #ifdef vwm_DEBUG_LEVEL
    ...
    if (new_params != NULL) {
      ...
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] failed to realloc params, size not increased!"));
      #endif
      return false;
    }
  }
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("Added Parameter:"),p->getID());
  #endif
  return true;
}
 ...
 * @access public
 */
ViraWMParameter** ViraWM::getParameters() {
  return _params;
}
...
// constructors
ViraWM::ViraWM(Print& consolePort):_debugPort(consolePort){
  ViraWMInit();
}
...
void ViraWM::ViraWMInit(){
  setMenu(_menuIdsDefault);
  if(_debug && _debugLevel >= vwm_DEBUG_DEV) debugPlatformInfo();
  _max_params = WIFI_MANAGER_MAX_PARAMS;
}
...
ViraWM::~ViraWM() {
  _end();
  // parameters
  // @todo below belongs to ViraWMparameter
  if (_params != NULL){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("freeing allocated params!"));
    #endif
    free(_params);
    _params = NULL;
  }
  ...
  // remove event
  // WiFi.onEvent(std::bind(&ViraWM::WiFiEvent,this,_1,_2));
  #ifdef ESP32
    WiFi.removeEvent(vwm_event_id);
  #endif

  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("unloading"));
  #endif
}
 ...
 */
boolean ViraWM::autoConnect(char const *apName, char const *apPassword) {
  #ifdef vwm_DEBUG_LEVEL
  ...
  // if(wifiIsSaved){
    ...
    if(!WiFi.enableSTA(true)){
      ...
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_ERROR,F("[FATAL] Unable to enable wifi!"));
      #endif
      return false;
    }
    ...
    if (WiFi.status() == WL_CONNECTED){
      connected = true;
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(F("AutoConnect*WiFi-Already-Connected"));
      #endif
      setSTAConfig();
    ...
    if(connected || connectWifi(_defaultssid, _defaultpass) == WL_CONNECTED){
      ...
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Connected in"),(String)((millis()-_startconn)) + " ms");
      ...
      if(_hostname != ""){
        #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_DEV,F("hostname: STA: "),getWiFiHostname());
        #endif
      }
    ...
    #ifdef vwm_DEBUG_LEVEL
	DEBUG_WM(F("AutoConnect*FAILED*") + String(millis() - _startconn) + "*ms*");
	delay(500);
	#endif
  ...
  if (!_enableConfigPortal) {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("enableConfigPortal: FALSE, skipping "));
    #endif
...
bool ViraWM::setupHostname(bool restart){
  if(_hostname == "") {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("No Hostname to set"));
    #endif
    return false;
  } 
  else {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("Setting Hostnames: "),_hostname);
    #endif
  }
  bool res = true;
  #ifdef ESP8266
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("Setting WiFi hostname"));
    #endif
    res = WiFi.hostname(_hostname.c_str());
    // #ifdef ESP8266MDNS_H
    #ifdef vwm_MDNS
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Setting MDNS hostname, tcp 80"));
      #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
  if(!res)DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] hostname: set failed!"));
  #endif

  if(restart && (WiFi.status() == WL_CONNECTED)){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("reconnecting to set new hostname"));
    #endif
...
bool ViraWM::startAP(){
  ...
  //DEBUG_WM(F("StartAP with SSID:"), String(_apName) + "*");
  DEBUG_WM(F("StartAP-with-SSID*") + String(_apName) + "*");
  #endif
    ...
    if(!WiFi.enableAP(true)) {
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] enableAP failed!"));
      #endif
      return false;
    }
  ...
  if (_ap_static_ip) {
    ...
    if(!WiFi.softAPConfig(_ap_static_ip, _ap_static_gw, _ap_static_sn)){
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] softAPConfig failed!"));
      #endif
    }
  }
  ...
  if(channel>0){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("*Starting AP on channel:*"),channel);
    #endif
  }
  ...
  if (_apPassword != "") {
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("*AP has anonymous access!*"));    
    #endif
    if(channel>0){
  ...
  if(_debugLevel >= vwm_DEBUG_DEV) debugSoftAPConfig();

  // @todo add softAP retry here to dela with unknown failures
  
  delay(500); // slight delay to make sure we get an AP IP
  #ifdef vwm_DEBUG_LEVEL
  if(!ret) DEBUG_WM(vwm_DEBUG_ERROR,F("*[ERROR] There was a problem starting the AP*"));
  //DEBUG_WM(F("*AP-IP-ADDR:*"),WiFi.softAPIP());
  DEBUG_WM(String(F("AP-IP-ADDR*")) + WiFi.softAPIP().toString() + "*");
  #endif
  ...
  // set ap hostname
    ...
    if(ret && _hostname != ""){
      ...
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("*setting softAP Hostname:*"),_hostname,"*");
      if(!res)DEBUG_WM(vwm_DEBUG_ERROR,F("*[ERROR] hostname: AP set failed!*"));
      DEBUG_WM(vwm_DEBUG_DEV,F("*hostname: AP: *"),WiFi.softAPgetHostname(),"*");
      #endif
   }
  #endif
 ...
 * @return {[type]} [description]
 */
void ViraWM::stopWebPortal() {
  if(!configPortalActive && !webPortalActive) return;
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("*Stopping Web Portal*"));  
  #endif
  webPortalActive = false;
  shutdownConfigPortal();
}

boolean ViraWM::configPortalHasTimeout(){
    ...
    if(_configPortalTimeout == 0 || (_apClientCheck && (WiFi_softap_num_stations() > 0))){
      ...
      if(millis() - timer > logintvl){
        timer = millis();
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("*NUM CLIENTS:* "),(String)WiFi_softap_num_stations());
        #endif
      }
    ...
    if(millis() > _configPortalStart + _configPortalTimeout){
      ...
      if((millis() - timer) > logintvl){
        timer = millis();
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("*Portal Timeout In*"),(String)((_configPortalStart + _configPortalTimeout-millis())/1000) + (String)F(" seconds*"));
        #endif
      }
    }
...
void ViraWM::setupHTTPServer(){
  ...
  if(_httpPort != 80) {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("*http server started with custom port: *"),_httpPort); // @todo not showing ip
    #endif
  }

  server.reset(new vwm_WebServer(_httpPort));
  ...
  if ( _webservercallback != NULL) {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("*[CB] _webservercallback calling*"));
    #endif
  ...
  server->on(vwm_G(R_root),       std::bind(&ViraWM::handleRoot, this));
  server->on(vwm_G(R_wifi),       std::bind(&ViraWM::handleWifi, this, true));
  server->on(vwm_G(R_wifinoscan), std::bind(&ViraWM::handleWifi, this, false));
  server->on(vwm_G(R_wifisave),   std::bind(&ViraWM::handleWifiSave, this));
  server->on(vwm_G(R_info),       std::bind(&ViraWM::handleInfo, this));
  server->on(vwm_G(R_param),      std::bind(&ViraWM::handleParam, this));
  server->on(vwm_G(R_paramsave),  std::bind(&ViraWM::handleParamSave, this));
  server->on(vwm_G(R_restart),    std::bind(&ViraWM::handleReset, this));
  server->on(vwm_G(R_exit),       std::bind(&ViraWM::handleExit, this));
  server->on(vwm_G(R_close),      std::bind(&ViraWM::handleClose, this));
  server->on(vwm_G(R_erase),      std::bind(&ViraWM::handleErase, this, false));
  server->on(vwm_G(R_status),     std::bind(&ViraWM::handleWiFiStatus, this));
  server->onNotFound (std::bind(&ViraWM::handleNotFound, this));
  
  server->on(vwm_G(R_update), std::bind(&ViraWM::handleUpdate, this));
  server->on(vwm_G(R_updatedone), HTTP_POST, std::bind(&ViraWM::handleUpdateDone, this), std::bind(&ViraWM::handleUpdating, this));
  
  server->begin(); // Web server start
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("*HTTP server started*"));
  #endif
}

void ViraWM::setupDNSD(){
  ...
  DEBUG_WM(vwm_DEBUG_DEV,F("*dns server started with ip: *"),WiFi.softAPIP()); // @todo not showing ip
  #endif
  dnsServer->start(DNS_PORT, F("*"), WiFi.softAPIP());
}

void ViraWM::setupConfigPortal() {
  setupHTTPServer();
  _lastscan = 0; // reset network scan cache
  if(_preloadwifiscan) WiFi_scanNetworks(true,true); // preload wifiscan , async
}

boolean ViraWM::startConfigPortal() {
  String ssid = getDefaultAPName();
 ...
 * @return {[type]}      [description]
 */
boolean  ViraWM::startConfigPortal(char const *apName, char const *apPassword) {
  _begin();

  if(configPortalActive){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("*Starting Config Portal FAILED, is already running*"));
    #endif    
    return false;
  }
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("*Starting Config Portal*"));
  #endif
  ...
  if(_disableSTA || (!WiFi.isConnected() && _disableSTAConn)){
    ...
    #ifdef vwm_DISCONWORKAROUND
      WiFi.mode(WIFI_AP_STA);
    #endif
    WiFi_Disconnect();
    WiFi_enableSTA(false);
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("*Disabling STA*"));
    #endif
  }
  else {
    // WiFi_enableSTA(true);
  ...
  // start access point
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("Enabling AP"));
  #endif
  startAP();
  WiFiSetCountry();
  ...
  if ( _apcallback != NULL) {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("[CB] _apcallback calling"));
    #endif
    _apcallback(this);
  }
  ...
  // init configportal
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("setupConfigPortal"));
  #endif
  setupConfigPortal();

  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("setupDNSD"));
  #endif  
  setupDNSD();
  ...
  if(!_configPortalIsBlocking){
    #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Config Portal Running, non blocking (processing)"));
      if(_configPortalTimeout > 0) DEBUG_WM(vwm_DEBUG_VERBOSE,F("Portal Timeout In"),(String)(_configPortalTimeout/1000) + (String)F(" seconds"));
    #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("Config Portal Running, blocking, waiting for clients..."));
    if(_configPortalTimeout > 0) DEBUG_WM(vwm_DEBUG_VERBOSE,F("Portal Timeout In"),(String)(_configPortalTimeout/1000) + (String)F(" seconds"));
  #endif
  ...
  while(1){
    ...
    if(configPortalHasTimeout() || abort){
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_DEV,F("configportal loop abort"));
      #endif
      shutdownConfigPortal();
      result = abort ? portalAbortResult : portalTimeoutResult; // false, false
      if (_configportaltimeoutcallback != NULL) {
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("[CB] config portal timeout callback"));
        #endif
    ...
    if(state != WL_IDLE_STATUS){
        ...
        DEBUG_WM(vwm_DEBUG_DEV,F("configportal loop break"));
        break;
    }
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_NOTIFY,F("config portal exiting"));
  #endif
  return result;
}
 ...
 */
boolean ViraWM::process(){
    // process mdns, esp32 not required
    #if defined(vwm_MDNS) && defined(ESP8266)
    MDNS.update();
    #endif
    ...
    if(webPortalActive || (configPortalActive && !_configPortalIsBlocking)){
      ...
      if(_allowExit && (configPortalHasTimeout() || abort)){
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_DEV,F("process loop abort"));
        #endif
        webPortalActive = false;
        shutdownConfigPortal();
        if (_configportaltimeoutcallback != NULL) {
 ...
 * @return {[type]} [description]
 */
uint8_t ViraWM::processConfigPortal(){
    if(configPortalActive){
      //DNS handler
    ...
    if(connect) {
      connect = false;
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("processing save"));
      #endif
      ...
      if(_ssid == ""){
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("No ssid, skipping wifi save"));
        #endif
      }
      else{
        ...
        if (res || (!_connectonsave)) {
          ...
          if ( _savewificallback != NULL) {
            #ifdef vwm_DEBUG_LEVEL
            DEBUG_WM(vwm_DEBUG_VERBOSE,F("[CB] _savewificallback calling"));
            #endif
        ...
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] Connect to new AP Failed"));
        #endif
      }
      ...
      if (_shouldBreakAfterConfig) {
        ...
        if ( _savewificallback != NULL) {
          #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_VERBOSE,F("[CB] WiFi/Param save callback"));
          #endif
          _savewificallback(); // @CALLBACK
        }
        if(_disableConfigPortal) shutdownConfigPortal();
        ...
        WiFi_enableSTA(false);
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("Processing - Disabling STA"));
        #endif
      }
      else{
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("Portal is non blocking - remaining open"));
        #endif        
      }
    }
 ...
 */
bool ViraWM::shutdownConfigPortal(){
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("shutdownConfigPortal"));
  #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
  if(!ret)DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] disconnect configportal - softAPdisconnect FAILED"));
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("restoring usermode"),getModeString(_usermode));
  #endif
  delay(1000);
  ...
  if(WiFi.status()==WL_IDLE_STATUS){
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFi Reconnect, was idle"));
    #endif
  }
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("wifi status:"),getWLStatusString(WiFi.status()));
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("wifi mode:"),getModeString(WiFi.getMode()));
  #endif
  configPortalActive = false;
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("configportal closed"));
  _end();
  return ret;
}
...
uint8_t ViraWM::connectWifi(String ssid, String pass, bool connect) {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("Connecting as wifi client..."));
  #endif
  uint8_t retry = 1;
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("Connection result:"),getWLStatusString(connRes));
  #endif
  retry++;
}
 ...
 */
bool ViraWM::wifiConnectNew(String ssid, String pass,bool connect){
  bool ret = false;
  #ifdef vwm_DEBUG_LEVEL
  // DEBUG_WM(vwm_DEBUG_DEV,F("CONNECTED: "),WiFi.status() == WL_CONNECTED ? "Y" : "NO");
  DEBUG_WM(F("Connecting to NEW AP:"),ssid);
  DEBUG_WM(vwm_DEBUG_DEV,F("Using Password:"),pass);
  #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
  if(!ret) DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] wifi begin failed"));
  #endif
  return ret;
}
 ...
 * @return bool success
 */
bool ViraWM::wifiConnectDefault(){
  bool ret = false;

  #ifdef vwm_DEBUG_LEVEL
  //DEBUG_WM(F("Connecting to SAVED AP:"),WiFi_SSID(true));
  //DEBUG_WM(vwm_DEBUG_DEV,F("Using Password:"),WiFi_psk(true));
  
  String ssid = WiFi_SSID(true); 
  ssid.trim(); 
  DEBUG_WM(String(F("Connecting-to-SAVED-AP*")) + ssid + "*");
  
  String psk = WiFi_psk(true);
  psk.trim(); 
  DEBUG_WM(vwm_DEBUG_DEV, F("Using-Password*"), (psk + "*"));
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Mode after delay: "),getModeString(WiFi.getMode()));
  if(!ret) DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] wifi enableSta failed"));
  #endif
 ...
 * @return bool success
 */
bool ViraWM::setSTAConfig(){
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("STA static IP:"),_sta_static_ip);  
  #endif
  bool ret = true;
  if (_sta_static_ip) {
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Custom static IP/GW/Subnet/DNS"));
      #endif
    if(_sta_static_dns) {
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Custom static DNS"));
      #endif
      ret = WiFi.config(_sta_static_ip, _sta_static_gw, _sta_static_sn, _sta_static_dns);
    }
    else {
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Custom STA IP/GW/Subnet"));
      #endif
      ret = WiFi.config(_sta_static_ip, _sta_static_gw, _sta_static_sn);
    }

      #ifdef vwm_DEBUG_LEVEL
      if(!ret) DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] wifi config failed"));
      else DEBUG_WM(F("STA IP set:"),WiFi.localIP());
      #endif
  } 
  else {
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("setSTAConfig static ip not set, skipping"));
      #endif
  }
  return ret;
}
...
void ViraWM::updateConxResult(uint8_t status){
  ...
  _lastconxresult = status;
      ...
      if(_lastconxresult == WL_CONNECT_FAILED || _lastconxresult == WL_DISCONNECTED){
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_DEV,F("lastconxresulttmp:"),getWLStatusString(_lastconxresulttmp));            
        #endif
    ...
    DEBUG_WM(vwm_DEBUG_DEV,F("lastconxresult:"),getWLStatusString(_lastconxresult));
    #endif
}

 
uint8_t ViraWM::waitForConnectResult() {
  #ifdef vwm_DEBUG_LEVEL
  if(_connectTimeout > 0) DEBUG_WM(vwm_DEBUG_DEV,_connectTimeout,F("ms connectTimeout set")); 
  #endif
 ...
 */
uint8_t ViraWM::waitForConnectResult(uint32_t timeout) {
  if (timeout == 0){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(F("connectTimeout-not-set*WiFi-waitForConnectResult...*"));
    #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,timeout,F("ms timeout, waiting for connect..."));
  #endif
...
String ViraWM::getHTTPHead(String title){
  String page;
...
void ViraWM::HTTPSend(const String &content){
 ...
 */
void ViraWM::handleRoot() {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Root"));
  #endif
 ...
 */
void ViraWM::handleWifi(boolean scan) {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Wifi"));
  #endif
  handleRequest();
  String page = getHTTPHead(FPSTR(S_titlewifi)); // @token titlewifi
  if (scan) {
    #ifdef vwm_DEBUG_LEVEL
    // DEBUG_WM(vwm_DEBUG_DEV,"refresh flag:",server->hasArg(F("refresh")));
    #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Sent config page"));
  #endif
}
 ...
 */
void ViraWM::handleParam(){
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Param"));
  #endif
  handleRequest();
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Sent param page"));
  #endif
}


String ViraWM::getMenuOut(){
  String page;  
...
// bool ViraWM::WiFi_scanNetworksForAP(bool force){
//   WiFi_scanNetworks(force);
// }

void ViraWM::WiFi_scanComplete(int networksFound){
  _lastscan = millis();
  _numNetworks = networksFound;
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFi Scan ASYNC completed"), "in "+(String)(_lastscan - _startscan)+" ms");  
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFi Scan ASYNC found:"),_numNetworks);
  #endif
}

bool ViraWM::WiFi_scanNetworks(){
  return WiFi_scanNetworks(false,false);
}
 
bool ViraWM::WiFi_scanNetworks(unsigned int cachetime,bool async){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,async);
}
bool ViraWM::WiFi_scanNetworks(unsigned int cachetime){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,false);
}
bool ViraWM::WiFi_scanNetworks(bool force,bool async){
    #ifdef vwm_DEBUG_LEVEL
    // DEBUG_WM(vwm_DEBUG_DEV,"scanNetworks async:",async == true);
    // DEBUG_WM(vwm_DEBUG_DEV,_numNetworks,(millis()-_lastscan ));
    // DEBUG_WM(vwm_DEBUG_DEV,"scanNetworks force:",force == true);
    #endif
    ...
    if(_numNetworks == 0 && _autoforcerescan){
      DEBUG_WM(vwm_DEBUG_DEV,"NO APs found forcing new scan");
      force = true;
    }
    ...
    if(force){
      ...
      if(async && _asyncScan){
        #ifdef ESP8266
          #ifndef vwm_NOASYNC // no async available < 2.4.0
          #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFi Scan ASYNC started"));
          #endif
          using namespace std::placeholders; // for `_1`
          WiFi.scanNetworksAsync(std::bind(&ViraWM::WiFi_scanComplete,this,_1));
          #else
          DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFi Scan SYNC started"));
      ...
      if(res == WIFI_SCAN_FAILED){
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] scan failed"));
        #endif
      }  
      else if(res == WIFI_SCAN_RUNNING){
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] scan waiting"));
        #endif
        while(WiFi.scanComplete() == WIFI_SCAN_RUNNING){
          #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_ERROR,".");
          #endif
          delay(100);
        }
      ...
      _lastscan = millis();
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFi Scan completed"), "in "+(String)(_lastscan - _startscan)+" ms");
      #endif
      return true;
    }
    else {
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("Scan is cached"),(String)(millis()-_lastscan )+" ms ago");
      #endif
    }
    return false;
}

String ViraWM::ViraWM::getScanItemOut(){
    String page;
    ...
    if (n == 0) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          for (int j = i + 1; j < n; j++) {
            if (cssid == WiFi.SSID(indices[j])) {
              #ifdef vwm_DEBUG_LEVEL
              DEBUG_WM(vwm_DEBUG_VERBOSE,F("DUP AP:"),WiFi.SSID(indices[j]));
              #endif
      ...
      for (int i = 0; i < n; i++) {
        ...
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("AP: "),(String)WiFi.RSSI(indices[i]) + " " + (String)WiFi.SSID(indices[i]));
        #endif
        ...
        if (_minimumQuality == -1 || _minimumQuality < rssiperc) {
          ...
          #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_DEV,item);
          #endif
          page += item;
          delay(0);
        } else {
          #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_VERBOSE,F("Skipping , does not meet _minimumQuality"));
          #endif
        }
...
String ViraWM::getIpForm(String id, String title, String value){
...
String ViraWM::getStaticOut(){
  String page;
  if ((_staShowStaticFields || _sta_static_ip) && _staShowStaticFields>=0) {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("_staShowStaticFields"));
    #endif
...
String ViraWM::getParamOut(){
  String page;

  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("getParamOut"),_paramsCount);
  #endif
  ...
  if(_paramsCount > 0){
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      if (_params[i] == NULL || _params[i]->_length > 99999) {
        ...
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] ViraWMParameter is out of scope"));
        #endif
        return "";
      }
    }
...
void ViraWM::handleWiFiStatus(){
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP WiFi status "));
  #endif
  handleRequest();
  String page;
...
String messageBox(const String &message) {
    String html = "</br><p></br><p><center><div id='messageBox' style='"
                  "display: flex; "
    ...
    // دکمه بستن
    html += "<button onclick='hideMessageBox()' style='"
             "margin-left: 20px; "
    ...
    // دکمه بازگشت
    html += "<button onclick='goBack()' style='"
             "margin-left: 10px; "
    ...
    html += "<script>"
             ...
             "function goBack() { window.history.back(); }" // تابع جاوااسکریپت برای بازگشت
             "</script>";
    return html;
}


void ViraWM::handleWifiSave() {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP WiFi save "));
  DEBUG_WM(vwm_DEBUG_DEV,F("Method:"),server->method() == HTTP_GET  ? (String)FPSTR(S_GET) : (String)FPSTR(S_POST));
  #endif
  handleRequest();
  ...
  _svip = server->arg(F("si")).c_str();
  _svsb = server->arg(F("sb")).c_str();
  _svgw = server->arg(F("sg")).c_str();
  String isStatic = server->arg(F("static"));


  saveServerIP(_svip.c_str());
  saveGateway(_svgw.c_str()); 
  saveSubnet(_svsb.c_str());  

    if (isStatic.length() > 0) { 
        if (_svip.length() > 0 && _svsb.length() > 0 && _svgw.length() > 0) {
            // بررسی فرمت IP
            IPAddress ip, subnet, gateway;
            if (ip.fromString(_svip) && subnet.fromString(_svsb) && gateway.fromString(_svgw)) {
                // استفاده از IP استاتیک
                WiFi.config(ip, gateway, subnet);
                // ادامه کار...
            } else {
                // هشدار درباره فرمت اشتباه IP
                server->send(200, "text/html", messageBox("Invalid IP or Subnet or Gateway Format"));
                return;
            }
        } else {
            // هشدار درباره اطلاعات ناقص
            server->send(200, "text/html", messageBox("Please Enter IP,Subnet,Gateway Correctly"));
            return;
        }
    } else {
        // استفاده از DHCP
  ...
  if(_ssid == "" && _pass != ""){
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("Detected WiFi password change"));
    #endif    
  }
  ...
  DEBUG_WM(vwm_DEBUG_MAX,requestinfo);
  #endif
  ...
  if (server->arg(FPSTR(S_ip)) != "") {
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("static ip:"),ip);
    #endif
  }
  if (server->arg(FPSTR(S_gw)) != "") {
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("static gateway:"),gw);
    #endif
  }
  if (server->arg(FPSTR(S_sn)) != "") {
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("static netmask:"),sn);
    #endif
  }
  if (server->arg(FPSTR(S_dns)) != "") {
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("static DNS:"),dns);
    #endif
  }
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Sent wifi save page"));
  #endif

  connect = true; //signal ready to connect/reset process in processConfigPortal
}

void ViraWM::handleParamSave() {

  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Param save "));
  #endif
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Method:"),server->method() == HTTP_GET  ? (String)FPSTR(S_GET) : (String)FPSTR(S_POST));
  #endif
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Sent param save page"));
  #endif
}

void ViraWM::doParamSave(){
  ...
  if(_paramsCount > 0){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("Parameters"));
    DEBUG_WM(vwm_DEBUG_VERBOSE,FPSTR(D_HR));
    #endif
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,(String)_params[i]->getID() + ":",value);
      #endif
    }
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,FPSTR(D_HR));
 ...
 */
void ViraWM::handleInfo() {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Info"));
  #endif
  handleRequest();
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,F("Sent info page"));
  #endif
}

String ViraWM::getInfoData(String id){
  ...
  #endif
  #if defined(ESP32) && !defined(vwm_NOTEMP)
  else if(id==F("temp")){
  ...
  // }
    ...
    p.replace(FPSTR(T_1),FPSTR(vwm_VERSION_STR));
  }
 ...
 */
void ViraWM::handleExit() {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Exit"));
  #endif
  handleRequest();
 ...
 */
void ViraWM::handleReset() {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP Reset"));
  #endif
  handleRequest();
...
// void ViraWM::handleErase() {
//   handleErase(false);
// }
void ViraWM::handleErase(boolean opt) {
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_NOTIFY,F("<- HTTP Erase"));
  #endif
  handleRequest();
  ...
  if(ret) page += FPSTR(S_resetting); // @token resetting
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] WiFi EraseConfig failed"));
    #endif
  }
...
boolean ViraWM::captivePortal() {
  ...
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_DEV,"-> " + server->hostHeader());
  DEBUG_WM(vwm_DEBUG_DEV,"serverLoc " + serverLoc);
  #endif
  ...
  if (doredirect) {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- Request redirected to captive portal"));
    DEBUG_WM(vwm_DEBUG_DEV,"serverLoc " + serverLoc);
    #endif
...
void ViraWM::handleClose(){
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("Disabling Captive Portal"));
  stopCaptivePortal();
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- HTTP close"));
  #endif
  handleRequest();
...
void ViraWM::reportStatus(String &page){
  // updateConxResult(WiFi.status()); // @todo: this defeats the purpose of last result, update elsewhere or add logic here
  DEBUG_WM(vwm_DEBUG_DEV,F("[WIFI] reportStatus prev:"),getWLStatusString(_lastconxresult));
  DEBUG_WM(vwm_DEBUG_DEV,F("[WIFI] reportStatus current:"),getWLStatusString(WiFi.status()));
  String str;
  if (WiFi_SSID() != ""){
    if (WiFi.status()==WL_CONNECTED){
 ...
 * @return bool success
 */
bool ViraWM::disconnect(){
  if(WiFi.status() != WL_CONNECTED){
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("Disconnecting: Not connected"));
    #endif
    return false;
  }  
  #ifdef vwm_DEBUG_LEVEL
...
bool ViraWM::erase(bool opt){
  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM("Erasing");
  #endif

  #if defined(ESP32) && ((defined(vwm_ERASE_NVS) || defined(nvs_flash_h)))
    ...
    if(opt){
      ...
      err = nvs_flash_init();
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("nvs_flash_init: "),err!=ESP_OK ? (String)err : "Success");
      #endif
      err = nvs_flash_erase();
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("nvs_flash_erase: "), err!=ESP_OK ? (String)err : "Success");
      #endif
      return err == ESP_OK;
    }
    ...
    if(opt){
      ...
      if(SPIFFS.begin()){
        ...
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("spiffs erase: "),ret ? "Success" : "ERROR");
        #endif
      } else{
      #ifdef vwm_DEBUG_LEVEL
 ...
 */
void ViraWM::setTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void ViraWM::setConfigPortalTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void ViraWM::setConnectTimeout(unsigned long seconds) {
 ...
 */
void ViraWM::setConnectRetries(uint8_t numRetries){
 ...
 */
void ViraWM::setCleanConnect(bool enable){
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void ViraWM::setSaveConnectTimeout(unsigned long seconds) {
 ...
 */
void ViraWM::setSaveConnect(bool connect) {
 ...
 */
void ViraWM::setDebugOutput(boolean debug) {
  _debug = debug;
  if(_debug && _debugLevel == vwm_DEBUG_DEV) debugPlatformInfo();
  if(_debug && _debugLevel >= vwm_DEBUG_NOTIFY)DEBUG_WM((__FlashStringHelper *)vwm_VERSION_STR," D:"+String(_debugLevel));
}

void ViraWM::setDebugOutput(boolean debug, String prefix) {
  _debugPrefix = prefix;
  setDebugOutput(debug);
}

void ViraWM::setDebugOutput(boolean debug, vwm_debuglevel_t level) {
  _debugLevel = level;
  // _debugPrefix = prefix;
  setDebugOutput(debug);
}
 ...
 */
void ViraWM::setAPStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _ap_static_ip = ip;
  _ap_static_gw = gw;
  _ap_static_sn = sn;
}
 ...
 * @param {[type]} IPAddress sn [description]
 */
void ViraWM::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _sta_static_ip = ip;
  _sta_static_gw = gw;
  _sta_static_sn = sn;
}
 ...
 */
void ViraWM::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn, IPAddress dns) {
 ...
 */
void ViraWM::setMinimumSignalQuality(int quality) {
 ...
 */
void ViraWM::setBreakAfterConfig(boolean shouldBreak) {
 ...
 * @access public 
 * @param {[type]} void (*func)(ViraWM* wminstance)
 */
void ViraWM::setAPCallback( std::function<void(ViraWM*)> func ) {
  _apcallback = func;
}
 ...
 */
void ViraWM::setWebServerCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ViraWM::setSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ViraWM::setPreSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void(*func)(void)
 */
void ViraWM::setConfigResetCallback( std::function<void()> func ) {
    _resetcallback = func;
}
 ...
 * @param {[type]} void (*func)(void)
 */
void ViraWM::setSaveParamsCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ViraWM::setPreSaveParamsCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ViraWM::setPreOtaUpdateCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ViraWM::setConfigPortalTimeoutCallback( std::function<void()> func ) {
 ...
 * @param char element
 */
void ViraWM::setCustomHeadElement(const char* html) {
 ...
 * @param char element
 */
void ViraWM::setCustomMenuHTML(const char* html) {
 ...
 */
void ViraWM::setRemoveDuplicateAPs(boolean removeDuplicates) {
 ...
 */
void ViraWM::setConfigPortalBlocking(boolean shouldBlock) {
 ...
 */
void ViraWM::setRestorePersistent(boolean persistent) {
 ...
 */
void ViraWM::setShowStaticFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void ViraWM::setShowDnsFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void ViraWM::setShowPassword(boolean show){
  _showPassword = show;
}
 ...
 */
void ViraWM::setCaptivePortalEnable(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void ViraWM::setWiFiAutoReconnect(boolean enabled){
 ...
 */
void ViraWM::setAPClientCheck(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void ViraWM::setWebPortalClientCheck(boolean enabled){
 ...
 * @param boolean enabled [false]
 */
void ViraWM::setScanDispPerc(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void ViraWM::setEnableConfigPortal(boolean enable)
{
 ...
 * @param boolean enabled [true]
 */
void ViraWM::setDisableConfigPortal(boolean enable)
{
 ...
 */
bool  ViraWM::setHostname(const char * hostname){
  //@todo max length 32
  _hostname = String(hostname);
  return true;
}

bool  ViraWM::setHostname(String hostname){
  //@todo max length 32
  _hostname = hostname;
  return true;
}
 ...
 */
void ViraWM::setWiFiAPChannel(int32_t channel){
  _apChannel = channel;
}
 ...
 */
void ViraWM::setWiFiAPHidden(bool hidden){
  _apHidden = hidden;
}
 ...
 */
void ViraWM::setShowInfoErase(boolean enabled){
 ...
 * @param boolean enabled
 */
void ViraWM::setShowInfoUpdate(boolean enabled){
 ...
 */
bool ViraWM::getConfigPortalActive(){
 ...
 * @return bool true if active
 */
bool ViraWM::getWebPortalActive(){
  return webPortalActive;
}


String ViraWM::getWiFiHostname(){
  #ifdef ESP32
 ...
 */
void ViraWM::setTitle(String title){
  _title = title;
}
 ...
 */
void ViraWM::setMenu(const char * menu[], uint8_t size){
#ifdef vwm_DEBUG_LEVEL
  // DEBUG_WM(vwm_DEBUG_DEV,"setmenu array");
  #endif
  _menuIds.clear();
 ...
 */
void ViraWM::setMenu(std::vector<const char *>& menu){
#ifdef vwm_DEBUG_LEVEL
  // DEBUG_WM(vwm_DEBUG_DEV,"setmenu vector");
  #endif
  _menuIds.clear();
  ...
  #ifdef vwm_DEBUG_LEVEL
  // DEBUG_WM(vwm_DEBUG_DEV,getMenuOut());
  #endif
}
 ...
 * @since $dev
 */
void ViraWM::setParamsPage(bool enable){
 ...
 */
String ViraWM::getConfigPortalSSID() {
  return _apName;
}
 ...
 */
uint8_t ViraWM::getLastConxResult(){
 ...
 * @return string 
 */
String ViraWM::getDefaultAPName(){
 ...
 */
void ViraWM::setCountry(String cc){
  _wificountry = cc;
}
 ...
 */
void ViraWM::setClass(String str){
  _bodyClass = str;
}
 ...
 */
void ViraWM::setDarkMode(bool enable){
 ...
 */
void ViraWM::setHttpPort(uint16_t port){
  _httpPort = port;
}


bool ViraWM::preloadWiFi(String ssid, String pass){
  _defaultssid = ssid;
  _defaultpass = pass;
  return true;
}
 ...
 * @return String
 */
String ViraWM::getWiFiSSID(bool persistent){
 ...
 * @return String
 */
String ViraWM::getWiFiPass(bool persistent){
...
void ViraWM::DEBUG_WM(Generic text) {
  DEBUG_WM(vwm_DEBUG_NOTIFY,text,"");
}

template <typename Generic>
void ViraWM::DEBUG_WM(vwm_debuglevel_t level,Generic text) {
  if(_debugLevel >= level) DEBUG_WM(level,text,"");
}

template <typename Generic, typename Genericb>
void ViraWM::DEBUG_WM(Generic text,Genericb textb) {
  DEBUG_WM(vwm_DEBUG_NOTIFY,text,textb);
}

template <typename Generic, typename Genericb>
void ViraWM::DEBUG_WM(vwm_debuglevel_t level,Generic text,Genericb textb) {
  if(!_debug || _debugLevel < level) return;

  if(_debugLevel >= vwm_DEBUG_MAX){
    #ifdef ESP8266
    // uint32_t free;
    // uint16_t max;
    // uint8_t frag;
 ...
 * @return {[type]} [description]
 */
void ViraWM::debugSoftAPConfig(){
    
    #ifdef ESP8266
      softap_config config;
      wifi_softap_get_config(&config);
      #if !defined(vwm_NOCOUNTRY)
...
void ViraWM::debugPlatformInfo(){
  ...
  #elif defined(ESP32)
  #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(F("[SYS] WM version: "),      vwm_VERSION_STR);
...
int ViraWM::getRSSIasQuality(int RSSI) {
  int quality = 0;
...
/** Is this an IP? */
boolean ViraWM::isIp(String str) {
...
/** IP to String? */
String ViraWM::toStringIp(IPAddress ip) {
  String res = "";
  for (int i = 0; i < 3; i++) {
...
boolean ViraWM::validApPassword(){
  ...
  if (_apPassword != "") {
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,F("AccessPoint set password is VALID"));
    DEBUG_WM(vwm_DEBUG_DEV,"ap pass",_apPassword);
    #endif
  }
  return true;
}
 ...
 */
String ViraWM::htmlEntities(String str, bool whitespace) {
  str.replace("&","&amp;");
  str.replace("<","&lt;");
  str.replace(">","&gt;");
  str.replace("'","&#39;");
 ...
 * @return {[type]}         [description]
 */
String ViraWM::getWLStatusString(uint8_t status){
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String ViraWM::getWLStatusString(){
  uint8_t status = WiFi.status();
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String ViraWM::encryptionTypeStr(uint8_t authmode) {
#ifdef vwm_DEBUG_LEVEL
...
String ViraWM::getModeString(uint8_t mode){
  if(mode <= 3) return WIFI_MODES[mode];
  return FPSTR(S_NA);
}

bool ViraWM::WiFiSetCountry(){
  if(_wificountry == "") return false; // skip not set

  #ifdef vwm_DEBUG_LEVEL
  DEBUG_WM(vwm_DEBUG_VERBOSE,F("WiFiSetCountry to"),_wificountry);
  #endif
...
/*
  ...
  */
  ...
  // @todo move these definitions, and out of cpp `esp_wifi_set_country(&vwm_COUNTRY_US)`
  bool ret = true;
  ...
  if(WiFi.getMode() == WIFI_MODE_NULL){
      DEBUG_WM(vwm_DEBUG_ERROR,"[ERROR] cannot set country, wifi not init");        
  ...
  // that the country code will be overridden if connecting to a "foreign" AP)
    ...
    #ifndef vwm_NOCOUNTRY
    err = esp_wifi_set_country_code(_wificountry.c_str(), true);
    #else
    DEBUG_WM(vwm_DEBUG_ERROR,"[ERROR] esp wifi set country is not available");
    err = true;
    #endif
  }
  #ifdef vwm_DEBUG_LEVEL
    if(err){
      if(err == ESP_ERR_WIFI_NOT_INIT) DEBUG_WM(vwm_DEBUG_ERROR,"[ERROR] ESP_ERR_WIFI_NOT_INIT");
      else if(err == ESP_ERR_INVALID_ARG) DEBUG_WM(vwm_DEBUG_ERROR,"[ERROR] ESP_ERR_WIFI_ARG (invalid country code)");
      else if(err != ESP_OK)DEBUG_WM(vwm_DEBUG_ERROR,"[ERROR] unknown error",(String)err);
    }
  #endif
  ret = err == ESP_OK;
  
  #elif defined(ESP8266) && !defined(vwm_NOCOUNTRY)
       // if(WiFi.getMode() == WIFI_OFF); // exception if wifi not init!
       if(_wificountry == "US") ret = wifi_set_country((wifi_country_t*)&vwm_COUNTRY_US);
  else if(_wificountry == "JP") ret = wifi_set_country((wifi_country_t*)&vwm_COUNTRY_JP);
  else if(_wificountry == "CN") ret = wifi_set_country((wifi_country_t*)&vwm_COUNTRY_CN);
  #ifdef vwm_DEBUG_LEVEL
  else DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] country code not found"));
  #endif
  #endif
  
  #ifdef vwm_DEBUG_LEVEL
  if(ret) DEBUG_WM(vwm_DEBUG_VERBOSE,F("[OK] esp_wifi_set_country: "),_wificountry);
  else DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] esp_wifi_set_country failed"));  
  #endif
  return ret;
}

// set mode ignores WiFi.persistent 
bool ViraWM::WiFi_Mode(WiFiMode_t m,bool persistent) {
    bool ret;
    #ifdef ESP8266
...
bool ViraWM::WiFi_Mode(WiFiMode_t m) {
	return WiFi_Mode(m,false);
}

// sta disconnect without persistent
bool ViraWM::WiFi_Disconnect() {
    #ifdef ESP8266
      if((WiFi.getMode() & WIFI_STA) != 0) {
          bool ret;
          #ifdef vwm_DEBUG_LEVEL
          DEBUG_WM(vwm_DEBUG_DEV,F("WiFi station disconnect"));
          #endif
...
bool ViraWM::WiFi_enableSTA(bool enable,bool persistent) {
#ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("WiFi_enableSTA"),(String) enable? "enable" : "disable");
    #endif
    #ifdef ESP8266
      WiFiMode_t newMode;
      ...
      if((isEnabled != enable) || persistent) {
          if(enable) {
          #ifdef vwm_DEBUG_LEVEL
          	if(persistent) DEBUG_WM(vwm_DEBUG_DEV,F("enableSTA PERSISTENT ON"));
            #endif
...
bool ViraWM::WiFi_enableSTA(bool enable) {
	return WiFi_enableSTA(enable,false);
}

bool ViraWM::WiFi_eraseConfig() {
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_DEV,F("WiFi_eraseConfig"));
    #endif

    #ifdef ESP8266
      #ifndef vwm_FIXERASECONFIG 
...
uint8_t ViraWM::WiFi_softap_num_stations(){
  #ifdef ESP8266
...
bool ViraWM::WiFi_hasAutoConnect(){
  return WiFi_SSID(true) != "";
}

String ViraWM::WiFi_SSID(bool persistent) const{
...
String ViraWM::WiFi_psk(bool persistent) const {
    #ifdef ESP8266
    struct station_config conf;
...
#ifdef ESP32
  #ifdef vwm_ARDUINOEVENTS
  void ViraWM::WiFiEvent(WiFiEvent_t event,arduino_event_info_t info){
  #else
  void ViraWM::WiFiEvent(WiFiEvent_t event,system_event_info_t info){
    ...
    if(!_hasBegun){
      #ifdef vwm_DEBUG_LEVEL
        // DEBUG_WM(vwm_DEBUG_VERBOSE,"[ERROR] WiFiEvent, not ready");
      #endif
    ...
    #ifdef vwm_DEBUG_LEVEL
    // DEBUG_WM(vwm_DEBUG_VERBOSE,"[EVENT]",event);
    #endif
    if(event == ARDUINO_EVENT_WIFI_STA_DISCONNECTED){
    #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("[EVENT] WIFI_REASON: "),info.wifi_sta_disconnected.reason);
      #endif
      ...
      #ifdef vwm_DEBUG_LEVEL
      if(info.wifi_sta_disconnected.reason == WIFI_REASON_NO_AP_FOUND) DEBUG_WM(vwm_DEBUG_VERBOSE,F("[EVENT] WIFI_REASON: NO_AP_FOUND"));
      if(info.wifi_sta_disconnected.reason == WIFI_REASON_ASSOC_FAIL){
        if(_aggresiveReconn) _connectRetries+=4;
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("[EVENT] WIFI_REASON: AUTH FAIL"));
      }  
      #endif
      #ifdef esp32autoreconnect
      #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_VERBOSE,F("[Event] SYSTEM_EVENT_STA_DISCONNECTED, reconnecting"));
        #endif
        WiFi.reconnect();
      #endif
  }
...
void ViraWM::WiFi_autoReconnect(){
  #ifdef ESP8266
    ...
    // if(_wifiAutoReconnect){
      ...
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_VERBOSE,F("ESP32 event handler enabled"));
      #endif
      using namespace std::placeholders;
      if(vwm_event_id == 0) vwm_event_id = WiFi.onEvent(std::bind(&ViraWM::WiFiEvent,this,_1,_2));
    // }
  #endif
}

// Called when /update is requested
void ViraWM::handleUpdate() {
  #ifdef vwm_DEBUG_LEVEL
	DEBUG_WM(vwm_DEBUG_VERBOSE,F("<- Handle update"));
  #endif
	if (captivePortal()) return; // If captive portal redirect instead of displaying the page
...
void ViraWM::handleUpdating(){
  ...
  // UPLOAD START
    ...
    #ifdef vwm_DEBUG_LEVEL
    DEBUG_WM(vwm_DEBUG_VERBOSE,"[OTA] Update file: ", upload.filename.c_str());
    #endif
    ...
    // });
  	...
  	if (!Update.begin(maxSketchSpace)) { // start with max available size
        #ifdef vwm_DEBUG_LEVEL
        DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] OTA Update ERROR"), Update.getError());
        #endif
        error = true;
  ...
  // UPLOAD WRITE
      ...
      #ifdef vwm_DEBUG_LEVEL
      DEBUG_WM(vwm_DEBUG_ERROR,F("[ERROR] OTA Update WRITE ERROR"), Update.getError());
...
void ViraWM::handleUpdateDone() {
	DEBUG_WM(vwm_DEBUG_VERBOSE, F("<- Handle update done"));
	// if (captivePortal()) return; // If captive portal redirect instead of displaying the page
